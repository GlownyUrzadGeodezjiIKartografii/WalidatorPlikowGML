...
def przecieciaLiniiNapieciaNadziemna(layer):
    try:
        ...
        for f in linie.getFeatures():
            ...
            if kod == '0010_444':
                continue
            gml = f['gml_id'] if 'gml_id' in f.fields().names() else f.id()
            geom = f.geometry()
            if geom is None or not geom.isGeosValid() or rodzaj not in [
...
def zapisWspolrzednych(layer, plikGML):
    try:
        ...
        def init_layer_dict(attr_name):
            return {
                "Polygon": QgsVectorLayer("None?crs=EPSG:2180", attr_name + "_A", "memory"),
        ...
        for featureMember in root.findall('.//gml:featureMember', namespaces=ns):
            ...
            koniecWersjiObiektu = matches.findtext('.//ot:koniecWersjiObiektu', default=None, namespaces=ns)
            ...
            for g in geometrie:
                ...
                lista_bledow = []
                if geom is not None:
                    wsp_list = geom.text.strip().split()
                    ...
                    typ_geom = "Point"
                if lista_bledow:
                    trescBledu = '|'.join(lista_bledow)
                    bledy_bez_formatu = [b for b in lista_bledow if b != "błędny format współrzędnych"]
                    ...
                    if bledy_bez_formatu and typ_geom:
                        ...
                        if layer.name() not in klasy_krytyczne:
                            dummy = QgsFeature(layer.fields())
                            dummy.setAttribute("gml_id", f"{gml_id}|błędy krytyczne walidacji geometrii")  # ogólny komunikat
...
def kontrolaZgodnosciIdentyfikatoraUlicyZNazwa(layer, plikcsv_ulic, plikcsv_simc):
    ...
    for f in layer.getFeatures():
        ...
        if 'OT_PTPL_A' in layer.name():
            ...
            nazwa1 = str(f.attribute('placNazwa1')).strip() if f.attribute('placNazwa1') else 'NULL'
            nazwa2 = str(f.attribute('placNazwa2')).strip() if f.attribute('placNazwa2') else 'NULL'
        else:
            ...
            nazwa2 = str(f.attribute('ulicaNazwa2')).strip() if f.attribute('ulicaNazwa2') else 'NULL'
...
def kontrolaBialeZnaki(layer, plikGML):
    ...
    for featureMember in plikGML.findall('.//gml:featureMember', namespaces=ns):
        ...
        for pole in aktywne_atrybuty:
            ...
            if elem is not None and elem.text and elem.text != elem.text.strip():
    ...
    try:
        ...
        for f in kuko_layer.getFeatures():
            if get_attr(f, "rodzaj") == "zajezdnia lub baza transportowa" and f.geometry() and not f.geometry().isEmpty():
                index_kuko.insertFeature(f); kuko_dict[f.id()] = f
        ...
        for f in bubd_layer.getFeatures():
            if get_attr(f, "przewazajacaFunkcjaBudynku") in funkcje_zajezdniowe and f.geometry() and not f.geometry().isEmpty():
                index_bubd.insertFeature(f); bubd_dict[f.id()] = f
        if klasa == "OT_KUKO_A":
            for f in layer.getFeatures():
                ...
                if rodzaj != "zajezdnia lub baza transportowa":
                    if skrot == "zaj.": obiektyZbledami.append(f)
                    continue
                found = any(geom.contains(bubd_dict[fid].geometry()) for fid in index_bubd.intersects(geom.boundingBox()) if bubd_dict.get(fid))
                if (found and skrot != "zaj.") or (not found and skrot == "zaj."): obiektyZbledami.append(f)
        else:
            for f in layer.getFeatures():
                skrot = get_attr(f, "skrotKartograficzny")
                ...
                if not geom or funkcja not in funkcje_zajezdniowe or istnienie == "zniszczony":
                    if skrot == "zaj.": obiektyZbledami.append(f)
                    continue
                on_kuko = any(kuko_dict[fid].geometry().contains(geom) for fid in index_kuko.intersects(geom.boundingBox()) if kuko_dict.get(fid))
                if (not on_kuko and skrot != "zaj.") or (on_kuko and skrot == "zaj."): obiektyZbledami.append(f)
        return obiektyZbledami
    except Exception as e:
...
def kontrolaSkrot_el(layer):
    try:
        ...
        obiektyZbledami = []
        klasa = layer.name()[-9:]
        if klasa not in ['OT_KUPG_A', 'OT_BUBD_A']: return []
        layers = {l.name()[-9:]: l for l in QgsProject.instance().mapLayers().values()}
        kupg_layer, bubd_layer = layers.get("OT_KUPG_A"), layers.get("OT_BUBD_A")
        if not kupg_layer or not bubd_layer: return []
        ...
        for f in kupg_layer.getFeatures():
            if get_attr(f, "rodzaj") == "elektrownia" and f.geometry() and not f.geometry().isEmpty():
                index_kupg.insertFeature(f); kupg_dict[f.id()] = f
        if klasa == "OT_KUPG_A":
            for f in layer.getFeatures():
                skrot, rodzaj = get_attr(f, "skrotKartograficzny"), get_attr(f, "rodzaj")
                if not f.geometry() or rodzaj != "elektrownia":
                    if skrot == "el.": obiektyZbledami.append(f)
                    continue
                if skrot != "el.": obiektyZbledami.append(f)
        else:
            for f in layer.getFeatures():
                skrot, funkcja, istnienie, geom = get_attr(f, "skrotKartograficzny"), get_attr(f, "przewazajacaFunkcjaBudynku"), get_attr(f, "kategoriaIstnienia"), f.geometry()
                if not geom or funkcja != "elektrownia" or istnienie == "zniszczony":
                    if skrot == "el.": obiektyZbledami.append(f)
                    continue
                na_kupg = any(kupg_dict[fid].geometry().contains(geom) for fid in index_kupg.intersects(geom.boundingBox()) if kupg_dict.get(fid))
                if (not na_kupg and skrot != "el.") or (na_kupg and skrot == "el."): obiektyZbledami.append(f)
        return obiektyZbledami
    except Exception as e:
        ...
        if klasa == "OT_KUPG_A":
            for f in layer.getFeatures():
                ...
                if rodzaj == "elektrociepłownia":
                    if skrot != "elc.": obiektyZbledami.append(f)
                elif skrot == "elc.": obiektyZbledami.append(f)
        else:
            for f in layer.getFeatures():
                skrot, funkcja, istnienie, geom = get_attr(f, "skrotKartograficzny"), get_attr(f, "przewazajacaFunkcjaBudynku"), get_attr(f, "kategoriaIstnienia"), f.geometry()
                if not geom or geom.isEmpty(): continue
                if funkcja == "elektrociepłownia" and istnienie != "zniszczony":
                    on_kupg = any(kupg_dict[fid].geometry().contains(geom) for fid in index_kupg.intersects(geom.boundingBox()) if kupg_dict.get(fid))
                    if (not on_kupg and skrot != "elc.") or (on_kupg and skrot == "elc."): obiektyZbledami.append(f)
                elif skrot == "elc.": obiektyZbledami.append(f)
        return obiektyZbledami
    except Exception as e:
...
def kontrolaSkrot_gar(layer):
    try:
        ...
        ptzb_layer = next((l for l in QgsProject.instance().mapLayers().values() if l.name().endswith("OT_PTZB_A")), None)
        if not ptzb_layer: return []
        index_ptzb, ptzb_dict = QgsSpatialIndex(), {}
        for f in ptzb_layer.getFeatures():
            if get_attr(f, "rodzaj") == "jednorodzinna" and f.geometry() and not f.geometry().isEmpty():
                index_ptzb.insertFeature(f); ptzb_dict[f.id()] = f
        obiektyZbledami = []
        for f in layer.getFeatures():
            skrot, funkcja, istnienie, geom = get_attr(f, "skrotKartograficzny"), get_attr(f, "przewazajacaFunkcjaBudynku"), get_attr(f, "kategoriaIstnienia"), f.geometry()
            if not geom or geom.isEmpty(): continue
            if funkcja == "garaż" and istnienie != "zniszczony":
                on_ptzb = any(ptzb_dict[fid].geometry().contains(geom) for fid in index_ptzb.intersects(geom.boundingBox()) if ptzb_dict.get(fid))
                if (on_ptzb and skrot == "gar.") or (not on_ptzb and skrot != "gar."): obiektyZbledami.append(f)
            elif skrot == "gar.": obiektyZbledami.append(f)
        return obiektyZbledami
    except Exception as e:
...
def kontrolaSkrot_gaz(layer):
    try:
        def get_attr(f, attr):
            return str(f[attr] or "").strip().lower() if attr in f.fields().names() else "__MISSING__"
        obiektyZbledami, klasa = [], layer.name()[-9:]
        if klasa not in ['OT_KUPG_A', 'OT_BUBD_A', 'OT_BUIT_P']: return []
        layers = {l.name()[-9:]: l for l in QgsProject.instance().mapLayers().values()}
        kupg_layer = layers.get("OT_KUPG_A")
        if not kupg_layer: return []
        index_kupg, kupg_dict = QgsSpatialIndex(), {}
        for f in kupg_layer.getFeatures():
        ...
        if klasa == "OT_KUPG_A":
            for f in layer.getFeatures():
                skrot, rodzaj = get_attr(f, "skrotKartograficzny"), get_attr(f, "rodzaj")
                if rodzaj == "gazownia":
                    if skrot != "gaz.": obiektyZbledami.append(f)
                elif skrot == "gaz.": obiektyZbledami.append(f)
            ...
            for f in layer.getFeatures():
                ...
                if funkcja in ["zbiornik na gaz", "stacja gazowa"]:
                    on_kupg = any(kupg_dict[fid].geometry().contains(geom) for fid in index_kupg.intersects(geom.boundingBox()) if kupg_dict.get(fid))
                    if (not on_kupg and skrot != "gaz.") or (on_kupg and skrot == "gaz."): obiektyZbledami.append(f)
        elif klasa == "OT_BUIT_P":
            for f in layer.getFeatures():
                skrot, rodzaj, istnienie, info = get_attr(f, "skrotKartograficzny"), get_attr(f, "rodzaj"), get_attr(f, "kategoriaIstnienia"), get_attr(f, "informacjaDodatkowa")
...
def kontrolaSkrot_hod(layer):
    try:
        ...
        slowa_kluczowe = {"hodowl", "ferm", "drob", "trzod", "kurcz", "byd", "owiec", "kon", "ryb", "pszcz", "pasiek"}
        ...
        for f in kupg_layer.getFeatures():
            info = get_attr(f, "informacjaDodatkowa")
            nazwa = get_attr(f, "nazwa")
            if get_attr(f, "rodzaj") == "gospodarstwo hodowlane" and (
                any(s in info for s in slowa_kluczowe) or any(s in nazwa for s in slowa_kluczowe)
        ...
        if klasa == "OT_KUPG_A":
            for f in layer.getFeatures():
                ...
                nazwa = get_attr(f, "nazwa")
                geom = f.geometry()
                if not geom or rodzaj != "gospodarstwo hodowlane" or (
                    not any(s in info for s in slowa_kluczowe) and not any(s in nazwa for s in slowa_kluczowe)
                ):
                    if skrot == "hod.":
                        obiektyZbledami.append(f)
                    continue
                found = any(
                    geom.contains(bubd_dict[fid].geometry())
                    for fid in index_bubd.intersects(geom.boundingBox())
                    if bubd_dict.get(fid)
                )
                if (found and skrot != "hod.") or (not found and skrot == "hod."):
                    obiektyZbledami.append(f)
        elif klasa == "OT_BUBD_A":
            for f in layer.getFeatures():
                skrot = get_attr(f, "skrotKartograficzny")
                ...
                if not geom or funkcja != "budynek produkcyjny zwierząt hodowlanych" or istnienie == "zniszczony":
                    ...
                    continue
                on_kupg = any(
                    kupg_dict[fid].geometry().contains(geom)
                    for fid in index_kupg.intersects(geom.boundingBox())
                    if kupg_dict.get(fid)
                )
                if not on_kupg and skrot != "hod.":
                    obiektyZbledami.append(f)
                elif on_kupg and skrot == "hod.":
                    obiektyZbledami.append(f)
...
def kontrolaSkrot_kemp(layer):
    try:
        ...
        index_bubd, bubd_dict = QgsSpatialIndex(), {}
        for f in bubd_layer.getFeatures():
            if get_attr(f, "przewazajacaFunkcjaBudynku") == "domek kempingowy" and get_attr(f, "kategoriaIstnienia") != "zniszczony" and f.geometry() and not f.geometry().isEmpty():
                index_bubd.insertFeature(f)
                bubd_dict[f.id()] = f
        if klasa == "OT_KUHO_A":
            for f in layer.getFeatures():
                rodzaj = get_attr(f, "rodzaj")
                skrot = get_attr(f, "skrotKartograficzny")
                if rodzaj == "kemping" and skrot != "kemp.":
                    obiektyZbledami.append(f)
                elif rodzaj != "kemping" and skrot == "kemp.":
                    obiektyZbledami.append(f)
        elif klasa == "OT_BUBD_A":
            for f in layer.getFeatures():
                funkcja = get_attr(f, "przewazajacaFunkcjaBudynku")
                ...
                skrot = get_attr(f, "skrotKartograficzny")
                geom = f.geometry()
                if not geom or funkcja != "domek kempingowy" or istnienie == "zniszczony":
                    continue
                in_kuho = any(kuho_dict[fid].geometry().contains(geom)
                              for fid in index_kuho.intersects(geom.boundingBox())
                              if kuho_dict.get(fid))
                if in_kuho and skrot == "kemp.":
                    obiektyZbledami.append(f)
                elif not in_kuho and skrot != "kemp.":
                    obiektyZbledami.append(f)
...
def kontrolaSkrot_letn(layer):
    try:
        def get_attr(f, attr):
            return str(f[attr] or "").strip().lower() if attr in f.fields().names() else "__MISSING__"
        obiektyZbledami, klasa = [], layer.name()[-9:]
        if klasa not in ['OT_KUSK_A', 'OT_BUBD_A']: return []
        layers = {l.name()[-9:]: l for l in QgsProject.instance().mapLayers().values()}
        kusk_layer = layers.get("OT_KUSK_A")
        if not kusk_layer: return []
        ...
        if klasa == "OT_KUSK_A":
            for f in layer.getFeatures():
                skrot, rodzaj, geom = get_attr(f, "skrotKartograficzny"), get_attr(f, "rodzaj"), f.geometry()
                if not geom or rodzaj != "zespół domów letniskowych":
                    if skrot == "letn.": obiektyZbledami.append(f)
                elif skrot != "letn.": obiektyZbledami.append(f)
        elif klasa == "OT_BUBD_A":
            for f in layer.getFeatures():
                skrot = get_attr(f, "skrotKartograficzny")
                funkcja = get_attr(f, "przewazajacaFunkcjaBudynku")
                istnienie = get_attr(f, "kategoriaIstnienia")
                geom = f.geometry()
                if not geom or funkcja != "dom letniskowy" or istnienie == "zniszczony":
                    if skrot == "letn.": obiektyZbledami.append(f)
                    continue
                on_kusk = any(kusk_dict[fid].geometry().contains(geom) for fid in index_kusk.intersects(geom.boundingBox()) if kusk_dict.get(fid))
                if (not on_kusk and skrot != "letn.") or (on_kusk and skrot == "letn."): obiektyZbledami.append(f)
...
def kontrolaSkrot_osrWyp(layer):
    try:
        def get_attr(f, attr):
            return str(f[attr] or "").strip().lower() if attr in f.fields().names() else "__MISSING__"
        obiektyZbledami, klasa = [], layer.name()[-9:]
        if klasa not in ['OT_KUHO_A', 'OT_BUBD_A']: return []
        layers = {l.name()[-9:]: l for l in QgsProject.instance().mapLayers().values()}
        kuho_layer = layers.get("OT_KUHO_A")
        if not kuho_layer: return []
        index_kuho, kuho_dict = QgsSpatialIndex(), {}
        for f in kuho_layer.getFeatures():
        ...
        if klasa == "OT_KUHO_A":
            for f in layer.getFeatures():
                if not f.geometry() or get_attr(f, "rodzaj") != "ośrodek wypoczynkowy":
                    if get_attr(f, "skrotKartograficzny") == "ośr. wyp.": obiektyZbledami.append(f)
                    continue
                if get_attr(f, "skrotKartograficzny") != "ośr. wyp.": obiektyZbledami.append(f)
        elif klasa == "OT_BUBD_A":
            for f in layer.getFeatures():
                geom, skrot = f.geometry(), get_attr(f, "skrotKartograficzny")
                if not geom or get_attr(f, "przewazajacaFunkcjaBudynku") != "ośrodek szkoleniowo-wypoczynkowy" or get_attr(f, "kategoriaIstnienia") == "zniszczony":
                    if skrot == "ośr. wyp.": obiektyZbledami.append(f)
                    continue
                on_kuho = any(kuho_dict[fid].geometry().contains(geom) for fid in index_kuho.intersects(geom.boundingBox()) if fid in kuho_dict)
                if (not on_kuho and skrot != "ośr. wyp.") or (on_kuho and skrot == "ośr. wyp."): obiektyZbledami.append(f)
        return obiektyZbledami
    except Exception as e:
...
def kontrolaSkrot_parking(layer):
    try:
        ...
        if klasa == "OT_KUKO_A":
            for f in layer.getFeatures():
                if not f.geometry() or get_attr(f, "rodzaj") != "parking":
                    if get_attr(f, "skrotKartograficzny") == "p.": obiektyZbledami.append(f)
                    continue
                if get_attr(f, "skrotKartograficzny") != "p.": obiektyZbledami.append(f)
        elif klasa == "OT_BUBD_A":
            for f in layer.getFeatures():
                geom, skrot = f.geometry(), get_attr(f, "skrotKartograficzny")
                if not geom or get_attr(f, "przewazajacaFunkcjaBudynku") != "parking wielopoziomowy" or get_attr(f, "kategoriaIstnienia") == "zniszczony":
                    if skrot == "p.": obiektyZbledami.append(f)
                    continue
                on_kuko = any(kuko_dict[fid].geometry().contains(geom) for fid in index_kuko.intersects(geom.boundingBox()) if fid in kuko_dict)
                if (not on_kuko and skrot != "p.") or (on_kuko and skrot == "p."): obiektyZbledami.append(f)
        return obiektyZbledami
    except Exception as e:
...
def kontrolaSkrot_port(layer):
    try:
        ...
        index_bubd, bubd_dict = QgsSpatialIndex(), {}
        for f in bubd_layer.getFeatures():
            if get_attr(f, "przewazajacaFunkcjaBudynku") == "terminal portowy" and get_attr(f, "kategoriaIstnienia") != "zniszczony" and f.geometry() and not f.geometry().isEmpty():
                index_bubd.insertFeature(f)
                bubd_dict[f.id()] = f
        if klasa == "OT_KUKO_A":
            for f in layer.getFeatures():
                ...
                if not geom or rodzaj != "port wodny lub przystań":
                    if skrot == "port": obiektyZbledami.append(f)
                    continue
                found = any(geom.contains(bubd_dict[fid].geometry()) for fid in index_bubd.intersects(geom.boundingBox()) if fid in bubd_dict)
                if (found and skrot != "port") or (not found and skrot == "port"): obiektyZbledami.append(f)
        elif klasa == "OT_BUBD_A":
            for f in layer.getFeatures():
                geom, skrot = f.geometry(), get_attr(f, "skrotKartograficzny")
                if not geom or get_attr(f, "przewazajacaFunkcjaBudynku") != "terminal portowy" or get_attr(f, "kategoriaIstnienia") == "zniszczony":
                    if skrot == "port": obiektyZbledami.append(f)
                    continue
                on_kuko = any(kuko_dict[fid].geometry().contains(geom) for fid in index_kuko.intersects(geom.boundingBox()) if fid in kuko_dict)
                if (not on_kuko and skrot != "port") or (on_kuko and skrot == "port"): obiektyZbledami.append(f)
        return obiektyZbledami
    except Exception as e:
...
def kontrolaSkrot_rafin(layer):
    try:
        ...
        index_bubd, bubd_dict = QgsSpatialIndex(), {}
        for f in bubd_layer.getFeatures():
            if get_attr(f, "przewazajacaFunkcjaBudynku") == "rafineria" and get_attr(f, "kategoriaIstnienia") != "zniszczony" and f.geometry() and not f.geometry().isEmpty():
                index_bubd.insertFeature(f)
                bubd_dict[f.id()] = f
        if klasa == "OT_KUPG_A":
            for f in layer.getFeatures():
                geom, skrot, rodzaj = f.geometry(), get_attr(f, "skrotKartograficzny"), get_attr(f, "rodzaj")
                if not geom or rodzaj != "rafineria":
                    if skrot == "rafin.": obiektyZbledami.append(f)
                    continue
                found = any(geom.contains(bubd_dict[fid].geometry()) for fid in index_bubd.intersects(geom.boundingBox()) if fid in bubd_dict)
                if (found and skrot != "rafin.") or (not found and skrot == "rafin."): obiektyZbledami.append(f)
        elif klasa == "OT_BUBD_A":
            for f in layer.getFeatures():
                geom, skrot = f.geometry(), get_attr(f, "skrotKartograficzny")
                if not geom or get_attr(f, "przewazajacaFunkcjaBudynku") != "rafineria" or get_attr(f, "kategoriaIstnienia") == "zniszczony":
                    if skrot == "rafin.": obiektyZbledami.append(f)
                    continue
                on_kupg = any(kupg_dict[fid].geometry().contains(geom) for fid in index_kupg.intersects(geom.boundingBox()) if fid in kupg_dict)
                if (not on_kupg and skrot != "rafin.") or (on_kupg and skrot == "rafin."): obiektyZbledami.append(f)
        return obiektyZbledami
    except Exception as e:
...
def kontrolaSkrot_st(layer):
    try:
        ...
        layers = {l.name()[-9:]: l for l in QgsProject.instance().mapLayers().values()}
        kuko_layer, bubd_layer = layers.get("OT_KUKO_A"), layers.get("OT_BUBD_A")
        if not kuko_layer or not bubd_layer:
            return []
        index_kuko, kuko_dict = QgsSpatialIndex(), {}
        for f in kuko_layer.getFeatures():
            if get_attr(f, "rodzaj") == "stacja kolejowa" and f.geometry() and not f.geometry().isEmpty():
        ...
        if klasa == "OT_KUKO_A":
            ...
            for f in layer.getFeatures():
                ...
                istnienie = get_attr(f, "kategoriaIstnienia")
                geom = f.geometry()
                if funkcja != "dworzec kolejowy" or istnienie == "zniszczony" or not geom:
                    if skrot == "st.":
                        obiektyZbledami.append(f)
                    continue
                on_kuko = any(
                    kuko_dict[fid].geometry().contains(geom)
                    for fid in index_kuko.intersects(geom.boundingBox())
                    if fid in kuko_dict
                )
                if (not on_kuko and skrot != "st.") or (on_kuko and skrot == "st."):
                    obiektyZbledami.append(f)
